...
const videos = [
  { 'name' : 'stranger' },
  {'name': 'First Person legendary bear attack. Red Dead hunting with Josiah..' }
 
];
// Create an instance of a db object for us to store our database in 
//media/media/reddead/_nathanchilton/_MG_8082_edited.jpg
let db;
...
function fetchVideoFromNetwork(video) {
  ...
  const mp4Blob = fetch(`/static/reddead/videos/${video.name}.mp4`).then(response => response.blob());
